...
import { ArrowLeft, CheckCircle, Circle, Clock, AlertTriangle, Pill, Activity, Stethoscope } from "lucide-react";
import { useState } from "react";
import clsx from "clsx";

// Mock Data for Doctors and Tasks
const DOCTORS = {
    'd1': { name: 'Dr. Emily Chen', specialization: 'Orthopedics' },
    'd2': { name: 'Dr. Michael Ross', specialization: 'Cardiology' }
};
...
const DoctorChecklist = () => {
    ...
    const [tasks, setTasks] = useState(INITIAL_TASKS.filter(t => t.doctorId === doctorId));

    // Fallback if doctor not found (or in real app, fetch)
    const doctor = DOCTORS[doctorId] || { name: 'Unknown Doctor', specialization: 'General' };
    ...
    return (
        <div className="min-h-screen bg-slate-50 p-6 font-sans pb-24">
            <header className="flex items-start mb-8 gap-4">
                <button
                    onClick={() => navigate(-1)}
                    className="p-2 bg-white rounded-full shadow-sm text-gray-600 hover:text-gray-900 transition-colors mt-1"
                >
                ...
                <div>
                    <div className="flex items-center gap-2 text-primary-600 mb-1">
                        ...
                        <span className="text-sm font-bold uppercase tracking-wider">{doctor.specialization}</span>
                    </div>
                    <h1 className="text-2xl font-bold text-gray-900">{doctor.name}</h1>
                    <p className="text-gray-500 text-sm">Assigned tasks & prescriptions.</p>
                </div>
            </header>
            ...
            <div className="space-y-4 max-w-lg mx-auto">
                {tasks.length === 0 ? (
                    <div className="text-center py-12 bg-white rounded-2xl border border-dashed border-gray-200">
                        ...
                        <p className="text-gray-500">No active tasks from this doctor.</p>
                    </div>
                ) : tasks.map((task) => (
                    <div
                        key={task.id}
